...
// 会话级自动压缩：回合结束检查用量，超过会话阈值时自动 compact。
// 阈值通过 settings 命名空间 auto-compact 持久化（client 用 api.settings 读写）。
...
export function apply(ctx) {
  ...
  try {
    ...
    const off = ctx.settings.register('auto-compact', schema, { base: { thresholds: {} } })
    ...
    try {
      ...
      const off = webServer.register({
        ...
        handler: async (req, res) => {
          ...
          let method = ''
          try { method = new URL(req.url || '/', 'http://dsh.internal').pathname.slice('/auto-compact/api/'.length) } catch (e) { method = '' }
          ...
          try {
            if (method === 'thresholds.get') {
              const resolved = ctx.settings.get('auto-compact')
            ...
            if (method === 'thresholds.set') {
              ...
              await ctx.settings.update('auto-compact', { thresholds: next })
