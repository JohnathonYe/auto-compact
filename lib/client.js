...
.ac-panel-hint{color:var(--dsw-alias-label-caption);margin-top:8px;font-size:11px;line-height:16px}`;
		...
		function apply(ctx) {
			...
			const readThresholds = async () => {
				try {
					const response = await fetch('/auto-compact/api/thresholds.get', { method: 'POST', headers: { 'content-type': 'application/json' }, body: '{}' });
			...
			const writeRatio = async (sessionId, ratio) => {
				try {
					await fetch('/auto-compact/api/thresholds.set', {
						method: 'POST',
